...
| **Operators** — 10 implemented | **Real** — all 10 implemented, unit-tested on fixture programs (signer, PDA seed, bump, authority, discriminator, CPI, close/rent, comparison flip, unchecked math, realloc check) |
...
- [x] Mutation engine: all 10 operators, unit-tested on fixture programs
...
- [x] 10 operators: added `unchecked_math` and `realloc_check_drop` — the former immediately surfaced 2 real test gaps on demo-vault.
- [x] CI/CD: build, test, clippy, fmt, cargo audit; demo-job publishes the report artifact (workflow in `.github/workflows/ci.yml`)
- [x] Third-party program evidence: engine runs on the unmodified `favorites` program from `solana-developers/program-examples` (`demo/favorites/ORIGIN.md`) — surfaced two real engine bugs, fixed in-tree
- [x] v0.1.0 tagged; release workflow packages + `cargo publish --dry-run` verified in CI (workflow: `.github/workflows/release.yml`)
- [ ] Next milestone (T2): full kill-path on ≥3 real third-party Anchor programs with hand-written LiteSVM test suites (favorites, escrow, one staking program); crates.io publish; hosted GitHub Action so any Anchor repo can `uses: subheeksh5599/mutanchor@v1`
