...
| **Demo program + published report** | **In progress** — `demo/demo-vault` build; report published to the live panel when the run lands |
...
Currently 18 tests are green across the engine (per-operator unit tests), the
report contract (publishable JSON shape, HTML, empty state), the CI gate, and
the CLI integration suite. The demo program (`demo/demo-vault`) carries its own
Anchor/LiteSVM test suite that exercises its instructions. Nothing on this site
or in this README is sample data; the report panel stays empty until a real run
exists.
...
- [ ] Demo: forked Anchor escrow/vault program with a published mutation report (in progress — `demo/demo-vault`)
