...
| **Demo program + published report** | **Real** — live run on `demo/demo-vault`: **72.2% mutation score (9 killed, 5 survived, 4 build-failed, 0 timeout, 18 mutants)**, published at [mutanchor.vercel.app/dashboard](https://mutanchor.vercel.app/dashboard) (raw: `/report.json`). The 5 survivors are 3× `authority_check_drop` (L32/L45/L73) + 2× `unchecked_math` (L34/L47) — the new `unchecked_math` operator immediately surfaced 2 real test gaps in the demo. Reproduced by CI in [run 33082543614](https://github.com/subheeksh5599/mutanchor/actions/runs/33082543614). |
| **Second in-repo program** | **Real** — `demo/demo-registry` (name-registry, 4 instructions, 10-test LiteSVM suite): **80.0% mutation score (8 killed, 3 survived, 4 build-failed, 0 timeout, 15 mutants)**. Published live at `/dashboard` (switch programs from the picker); raw at `/report-registry.json`. Different bug surface from demo-vault: transfer of ownership, checked_add pointer bound, unchecked-math survivor found. |
...
- **Demo coverage.** The published 72.2% score is on `demo/demo-vault` alone;
  ...
  LiteSVM suite) has its own published report at `/report-registry.json`.
...
- [x] Demo: `demo/demo-vault` (real Anchor program, 10-test LiteSVM suite) with a **live published mutation report** — [mutanchor.vercel.app/dashboard](https://mutanchor.vercel.app/dashboard), **72.2% score** (18 mutants, 9 killed / 5 survived / 4 build-failed / 0 timeout — reproduced by CI)
- [x] Second in-repo Anchor program: `demo/demo-registry` (name-registry with claim/transfer/set_pointer/reclaim, 10-test LiteSVM suite). **80.0% mutation score (15 mutants, 8k/3s/4bf)**. Own published report at `/report-registry.json`; switchable from the dashboard.
- [x] Hosted GitHub Action: `uses: subheeksh5599/mutanchor@v0.1.0` — one-line CI gate for any Anchor repo (composite action in `action.yml`).
