...
| **Demo program + published report** | **Real** — live run on `demo/demo-vault`: **81.2% mutation score (9 killed, 3 survived, 4 build-failed, 0 timeout, 16 mutants)**, published at [mutanchor.vercel.app/dashboard](https://mutanchor.vercel.app/dashboard) (raw: `/report.json`). The 3 survivors are all `authority_check_drop` at L32/L45/L73 — the same audit finding class. Reproduced by CI in [run 33076314990](https://github.com/subheeksh5599/mutanchor/actions/runs/33076314990). |
...
- **Demo coverage.** The published 81.2% score is on `demo/demo-vault` only.
...
| Mutants generated | 16 (0 equivalent/duplicate dropped) |
| Total wall time | **812 s (~13.5 min)** |
...
- [x] Demo: `demo/demo-vault` (real Anchor program, 10-test LiteSVM suite) with a **live published mutation report** — [mutanchor.vercel.app/dashboard](https://mutanchor.vercel.app/dashboard), **81.2% score** (16 mutants, 9 killed / 3 survived / 4 build-failed / 0 timeout — reproduced by CI)
